...
$sql_i="INSERT INTO manager(first_name,last_name,staff_id,postal_address,phone,email,username,password,date)
VALUES('$fname','$lname','$sid','$postal','$phone','$email','$user','$pas',NOW())";

if($conn->query($sql_i)===TRUE) {
 header("location:http:admin_manager.php");
}else{
    ...
    <div class="tabbed_area">  
		...
		*/
        ...
        while($row = $result->fetch_assoc()) {
				...
				?>
				<td><a href="update_manager.php?first_name=<?php echo $row['first_name']?>&last_name=<?php echo $row['last_name']?>&staff_id=<?php echo $row['staff_id']?>&postal_address=<?php echo $row['postal_address']?>$&phone=<?php echo $row['phone']?>&email=<?php echo $row['email']?>&username=<?php echo $row['username']?>&pasword=<?php echo $row['password']?>"><img src="images/update-icon.png" width="35" height="35" border="0" /></a></td>
