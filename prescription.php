...
<hr/>	
 ...
 $sql_v ="SELECT * FROM prescription";
			...
			$(document).ready(function()
	{
		...
		<form name="myform" onsubmit="return validateForm(this);" action="invoice.php" method="post" >
			<table width="200" height="106" border="0" >	
				...
				<tr><td>
				<select  class=\"input-small\" name=\"drug_name\" style=\"width:170px\" id=\"drug_name\">";
					<option value ="" >--select</option>
				<?php
						$option = "hi";
						 $sql_s="SELECT drug_name FROM stock";
						 $result =$conn->query($sql_s); 
					  if($result->num_rows>0){
					        while($row = $result->fetch_assoc()) {
					        	/*$option .= "<option value = ". $row['stock_id'] .">";
					        	$option .= $row['drug_name'] . "</option>";*/
							echo "<option  value = ". $row['stock_id'] . ">"
									. $row['drug_name'] .
									 "</option>";
								}
						}
